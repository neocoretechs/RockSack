...
try (final Options options = new Options();
  ...
  try (final RocksDB db = RocksDB.open(options, db_path)) {
    ...
    db.put(SerializedComparator.serializeObject("hello"), "world".getBytes());

    //final byte[] value = db.get("hello".getBytes());
    final byte[] value = db.get(SerializedComparator.serializeObject("hello"));
  ...
  try (final RocksDB db = RocksDB.open(options, db_path)) {
    ...
    byte[] value = db.get(SerializedComparator.serializeObject("hello"));
    ...
    for (int i = 1; i <= 9; ++i) {
      for (int j = 1; j <= 9; ++j) {
        ...
        db.put(SerializedComparator.serializeObject(String.format("%dx%d", i, j)),
      ...
      for (int j = 1; j <= 9; ++j) {
        System.out.format("%s ", new String(db.get(
            //String.format("%dx%d", i, j).getBytes())));
        		SerializedComparator.serializeObject(String.format("%dx%d", i, j)))));
      }
      System.out.println("");
    }
      ...
      for (int i = 10; i <= 19; ++i) {
        try (final WriteBatch batch = new WriteBatch()) {
          for (int j = 10; j <= 19; ++j) {
            ...
            batch.put(SerializedComparator.serializeObject(String.format("%dx%d", i, j)),
                String.format("%d", i * j).getBytes());
          }
    ...
    for (int i = 10; i <= 19; ++i) {
      for (int j = 10; j <= 19; ++j) {
        assert (new String(
        	...
        	db.get(SerializedComparator.serializeObject(String.format("%dx%d", i, j)))).equals(
            String.format("%d", i * j)));
        System.out.format("%s ", new String(db.get(
            //String.format("%dx%d", i, j).getBytes())));
        		SerializedComparator.serializeObject(String.format("%dx%d", i, j)))));
    ...
    value = db.get(SerializedComparator.serializeObject("1x1"));
    assert (value != null);
    //value = db.get("world".getBytes());
    value = db.get(SerializedComparator.serializeObject("world"));
    assert (value == null);
    //value = db.get(readOptions, "world".getBytes());
    value = db.get(readOptions, SerializedComparator.serializeObject("world"));
    assert (value == null);

    //final byte[] testKey = "asdf".getBytes();
    final byte[] testKey = SerializedComparator.serializeObject("asdf");
    ...
    len = db.get(SerializedComparator.serializeObject("asdfjkl;"), enoughArray);
    ...
    len = db.get(readOptions, SerializedComparator.serializeObject("asdfjkl;"), enoughArray);
    assert (len == RocksDB.NOT_FOUND);
    ...
    try {
      for (final HistogramType histogramType : HistogramType.values()) {
        //if (histogramType != HistogramType.HISTOGRAM_ENUM_MAX) {
    	  ...
    	  System.out.println("HISTOGRAM:"+histogramType);
          System.out.println(stats.getHistogramString(histogramType));
        //}
      }
    ...
    for (final byte[] value1 : values) {
      assert (value1 != null);
      //String skey = new String(keys.get(key++));
      String skey = (String) SerializedComparator.deserializeObject(keys.get(key++));
      String sval = new String(value1);
      System.out.println("Item:"+key+" Key:"+skey+" Value:"+sval);
    }
    System.out.println("-----");
    key = 0;
